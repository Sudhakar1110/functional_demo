...
(function () {
	...
	function hasOptions(listId) {
		var dl = document.getElementById(listId);
		return !!(dl && dl.querySelector("option"));
	}
	...
	document.getElementById("dr-form").addEventListener("submit", function (e) {
		...
		if (!hasOptions("customers-list") && !hasOptions("leads-list")) {
			portalAlert("No Customers or Leads exist in your system yet. Create at least one Customer (or Lead) in ERPNext first - or ask your administrator - before creating a demo request.", "err");
			return;
		}
		if (!customer && !lead) {
			portalAlert("Please select a Customer or a Lead before creating the demo request. Type in the Customer (or Lead) box and pick a suggestion from the list, then try again.", "err");
			return;
		}
		if (customer && !isKnown("customers-list", customer)) {
			portalAlert("Customer \"" + customer + "\" was not found. Please pick a customer from the suggestions list.", "err");
			return;
		}
	...
	{% if doc.functional_consultant and doc.status in ["Requested", "Assigned", "Scheduled", "Demo In Progress", "Demo Completed", "Follow-up Required"] %}
	...
	if (document.getElementById("btn-toggle-cancel-request")) {
		...
		document.getElementById("btn-cancel-request").addEventListener("click", function () {
			if (!window.confirm("Cancel Demo Request " + DOC_NAME + "? Open demo sessions will also be cancelled.")) return;
			var btn = this; btn.disabled = true;
