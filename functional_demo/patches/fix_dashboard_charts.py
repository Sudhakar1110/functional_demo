...
Frappe v15 requires `group_by_based_on` for 'Group By' Dashboard Charts.
Charts created from the initial fixtures omitted it, leaving the value empty
and crashing the chart renderer with:

    TypeError: argument of type 'NoneType' is not iterable
    (frappe.desk.doctype.dashboard_chart.dashboard_chart.get_group_by_chart_config)

This patch sets `group_by_based_on` directly on the affected chart records and
